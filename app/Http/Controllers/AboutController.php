...
class AboutController extends Controller
{
    ...
    public function save_about_vidyapati(Request $request) {
	    ...
	    if($validatedData->fails()) {
	      return redirect::back()->withErrors($validatedData);
    		...
    		return redirect('/about-vidyapati');
	    }
    }
	...
	public function about_ugna(){
		...
		$about_us = AboutUs::where('about_type','ugna')->get();
		$title = "Ugna";
		return view('backend.pages.aboutus.about-ugna',['about_us'=> $about_us,'title' => $title]);
	}

	public function add_about_ugna(){
		if(!\Auth::check()){
			return redirect('/login');
		}
		$title = "ugna";
		return view('backend.pages.aboutus.add-about-ugna',['title' => $title]);
	}

	public function save_about_ugna(Request $request){
		$validate = array(
	        'about_title' => 'required',
	        'about_description' => 'required|min:50',
	        'about_photo' => 'image|mimes:jpg,png,jpeg|max:2048',
	    ...
	    if($validatedData->fails()) {
	      return redirect::back()->withErrors($validatedData);
	    	...
	    	// about ugna insert array 
			$about_array = array();
			$about_array['about_title'] = $request->about_title;
			$about_array['about_description'] = $request->about_description;
			$about_array['about_type'] = 'ugna';
			$about_array['status'] = 'active';
	    	if($request->about_photo){
	    		$path = public_path('upload');
		    	if(!File::isDirectory($path)){
	    	...
	    	AboutUs::insert($about_array);
	    	session()->flash('success','success');
    		session()->flash('message',__('messages.add_about_ugna_success'));
    		return redirect('/about-ugna');
	    }
	}

	// delete about us ugna
	public function delete_about_ugna_description(Request $request) {
		$AboutUs = AboutUs::find($request['id']);
		$about_ugna_image_url = $AboutUs->about_photos;
		$image_image_array = explode('/', $about_ugna_image_url);
		@unlink(public_path('/upload').'/'.end($image_image_array));
		
		$AboutUs->delete();
		session()->flash('success','success');
    	session()->flash('message',__('messages.delete_about_ugna_success'));
    	return response()->json(['success' => 'true'], 200);
	}

	// update about ugna
	public function update_about_ugna(Request $request){
		$validate = array(
	        'about_title' => 'required',
	        'about_description' => 'required|min:50',
	        'about_photos' => 'image|mimes:jpg,png,jpeg|max:2048',
	    ...
	    if($validatedData->fails()) {
	      return redirect::back()->withErrors($validatedData);
	    	...
	    	$about->save();
	    	session()->flash('success','success');
	    	session()->flash('message',__('messages.update_about_ugna_success'));
	    	return redirect('/about-ugna');
		}	
